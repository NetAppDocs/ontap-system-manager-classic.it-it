...
+
Le subnet contengono un pool di indirizzi IP appartenenti alla stessa subnet Layer 3. Vengono creati utilizzando System Manager o il `network subnet create` comando.
...
** CLI: Utilizza il `network interface capacity show` comando e la capacità LIF supportati su ciascun nodo utilizzando il `network interface capacity details show` comando (a livello di privilegio avanzato).
...
* A partire da ONTAP 9.10.1, i `network interface` comandi CLI includono un `-rdma-protocols` parametro per le configurazioni NFS su RDMA. La creazione di interfacce di rete per le configurazioni NFS su RDMA è supportata in System Manager a partire da ONTAP 9.12.1. Per ulteriori informazioni, vedere xref:https://docs.netapp.com/us-en/ontap/nfs-rdma/configure-lifs-task.html[Configurare LIFS per NFS su RDMA^].
* A partire da ONTAP 9.11.1, il failover automatico iSCSI LIF è disponibile nelle piattaforme ASA (All-Flash SAN Array).
+
Il failover della LIF iSCSI viene abilitato automaticamente (la policy di failover è impostata su e il valore di indirizzamento automatico è `sfo-partner-only` impostato su `true`) in LIF iSCSI appena create se nella SVM specificata non esiste alcun LIF iSCSI o se tutti i LIF iSCSI esistenti nella SVM specificata sono già abilitati con il failover della LIF iSCSI.

+
Se dopo l'upgrade a ONTAP 9.11.1 o versione successiva, disponi di LIF iSCSI in una SVM che non è stata abilitata con la funzionalità di failover della LIF iSCSI e crei nuovi LIF iSCSI nella stessa SVM, i nuovi LIF iSCSI assumono la stessa policy di failover (`disabled`) dei LIF iSCSI esistenti nella SVM.

+
link:https://docs.netapp.com/us-en/ontap/san-admin/asa-iscsi-lif-fo-task.html["Failover LIF iSCSI per piattaforme ASA"^]
...
....
+
** `-home-node` È il nodo da cui ritorna la LIF quando il `network interface revert` comando viene eseguito sulla LIF.
+
Puoi anche specificare se LIF deve ripristinare automaticamente il nodo home e la porta home con l'opzione -auto-revert.

** `-home-port` Indica la porta fisica o logica alla quale la LIF ritorna quando il `network interface revert` comando viene eseguito nella LIF.
** È possibile specificare un indirizzo IP con le `-address` opzioni e `-netmask` oppure attivare l'assegnazione da una subnet con l' `-subnet_name`opzione .
...
** Se si assegnano gli indirizzi IP manualmente (senza utilizzare una subnet), potrebbe essere necessario configurare un percorso predefinito a un gateway se sono presenti client o controller di dominio su una subnet IP diversa. Ulteriori informazioni su `network route create` nella link:https://docs.netapp.com/us-en/ontap-cli/network-route-create.html["Riferimento comando ONTAP"^].
** `-auto-revert` Permette di specificare se un data LIF viene automaticamente riportato al nodo home in circostanze come l'avvio, modifiche allo stato del database di gestione o quando viene effettuata la connessione di rete. L'impostazione predefinita è `false`, ma è possibile impostarla su in `true` base ai criteri di gestione della rete nel proprio ambiente.
**  `-service-policy` A partire da ONTAP 9,5, puoi assegnare una policy di servizio per la LIF con l' `-service-policy`opzione. Quando viene specificata una policy di servizio per una LIF, questa viene utilizzata per creare un ruolo predefinito, una policy di failover e un elenco di protocolli dati per la LIF. In ONTAP 9.5, le policy di servizio sono supportate solo per i servizi peer di intercluster e BGP. In ONTAP 9.6, è possibile creare policy di servizio per diversi servizi di gestione e dati.
** `-data-protocol` Consente di creare una LIF che supporta i protocolli FCP o NVMe/FC. Questa opzione non è necessaria quando si crea un LIF IP.
...
+
Il `network ndp prefix show` comando è disponibile a livello di privilegi avanzati.

.. Utilizzare il formato `prefix::id` per creare manualmente l'indirizzo IPv6.
+
`prefix` è il prefisso appreso su varie interfacce.

+
Per derivare il `id`, scegliere un numero esadecimale a 64 bit casuale.
...
|===
.Esempi
Questo comando crea una LIF e specifica i valori di indirizzo IP e network mask usando `-address` i parametri e `-netmask`:
...
....
Il seguente comando crea una LIF NVMe/FC e specifica il `nvme-fc` protocollo dati:
